...
Blockly.ScratchBlocks.ProcedureUtils.callerMutationToDom = function () {
  ...
  container.setAttribute(
    "argumentdropdowns",
    JSON.stringify(this.argumentDropdowns_)
  );
...
Blockly.ScratchBlocks.ProcedureUtils.matchColours = function (colour1, ld) {
  ...
  const categorys = Object.values(Blockly.Categories);
  const maybeColours = Object.entries(Blockly.Colours).find(
    (v) => categorys.includes(v[0]) && v[1].primary.toLowerCase() === colour1
  );
  ...
  const c = parseInt(colour1.slice(1, 7), 16);
  const rgb = [c >> 16, (c >> 8) & 0x00ff, c & 0x0000ff];
...
Blockly.ScratchBlocks.ProcedureUtils.updateDisplay_ = function () {
  ...
  if (
    ...
    this.type === "procedures_declaration"
  ) {
    if (this.colours_) {
  ...
  if (!wasRendered && this.getReturn) {
    ...
    if (this.getReturn() === Blockly.PROCEDURES_CALL_TYPE_STATEMENT) {
      ...
      /*this.inputList.find(v => v.type == Blockly.NEXT_STATEMENT)*/ false
    ) {
...
Blockly.ScratchBlocks.ProcedureUtils.createAllInputs_ = function (
  ...
  let i, component, labelText, input, dropdownOptions;
  ...
  let procComponents = this.procCode_.split(/(?=[^\\]%[nbdoasclr])/);
  ...
  for (i = 0; (component = procComponents[i]); i++) {
    if (component.substring(0, 1) == "%" && component.substring(1, 2) != "l") {
      ...
      if (argumentType === "c") {
        input = this.appendStatementInput(id).setCheck(
          this.type == "procedures_prototype"
            ? "argumentReporterCommand"
            : "normal"
        );
      } else {
        ...
        if (argumentType == "b") {
          ...
          argumentType == "d" &&
          this.argumentDropdowns_[dropdownCount]
        ) {
          dropdownOptions = this.argumentDropdowns_[dropdownCount].map((o) => [
            o,
            o,
          ]);
          dropdownCount++;
...
Blockly.ScratchBlocks.ProcedureUtils.createArgumentReporter_ = function (
  ...
  if (argumentType == "n" || argumentType == "s" || argumentType == "d") {
    ...
    blockType = "argument_reporter_object";
  } else if (argumentType == "a") {
    blockType = "argument_reporter_array";
  } else if (argumentType == "r") {
    blockType = "argument_reporter_color";
...
Blockly.ScratchBlocks.ProcedureUtils.updateDropdowns_ = function () {
  let i, input, currentValue;
  let dropdownCount = 0;
  for (i = 0; (input = this.inputList[i]); i++) {
    const target = input.connection && input.connection.targetBlock();
    if (target && target.type == "procedures_dropdown") {
      const options = this.argumentDropdowns_[dropdownCount++];
      const field = target.getField("DROPDOWN_VALUE");
...
Blockly.ScratchBlocks.ProcedureUtils.populateArgumentOnDeclaration_ = function (
  ...
  if (type === "d" && dropdownOptions) {
    const field = argumentEditor.inputList[0].fieldRow[0];
...
Blockly.ScratchBlocks.ProcedureUtils.createArgumentEditor_ = function (
  ...
  try {
    if (argumentType == "n" || argumentType == "s") {
      ...
      newBlock = this.workspace.newBlock("argument_editor_object");
    } else if (argumentType == "a") {
      newBlock = this.workspace.newBlock("argument_editor_array");
    } else if (argumentType == "r") {
      newBlock = this.workspace.newBlock("argument_editor_color");
    } else if (argumentType == "d") {
      newBlock = this.workspace.newBlock("argument_editor_dropdown");
    } else if (argumentType == "c") {
      newBlock = this.workspace.newBlock("argument_editor_statement");
    }
...
Blockly.ScratchBlocks.ProcedureUtils.updateDeclarationProcCode_ = function (
  ...
  for (let i = 0; i < this.inputList.length; i++) {
    ...
    if (input.type == Blockly.DUMMY_INPUT) {
      this.procCode_ +=
        (opt_separateLabels ? "%l " : "") + input.fieldRow[0].getValue();
      ...
      if (target.type == "argument_editor_boolean") {
        ...
        this.procCode_ += "%d";
        const options = target.inputList[0].fieldRow[0].getOptions();
 ...
 * @public
 */
Blockly.ScratchBlocks.ProcedureUtils.shiftFieldCallback = function (
  field,
  direction
) {
  const declaration = this.parentBlock_ || this;
  if (
  ...
  let oldIndex = -1;
  for (let i = 0; i < declaration.inputList.length; i++) {
    const input = declaration.inputList[i];
    const containsField = input.connection
  ...
  const reorderedInputs = declaration.inputList.slice();
  const movedInput = reorderedInputs.splice(oldIndex, 1)[0];
  reorderedInputs.splice(newIndex, 0, movedInput);
  if (reorderedInputs[0].type == Blockly.NEXT_STATEMENT) {
    return;
  }
  declaration.inputList.splice.apply(
    declaration.inputList,
  ...
  const focusedInput = declaration.inputList[newIndex];
...
Blockly.ScratchBlocks.ProcedureUtils.getSelectedInputIndex_ = function () {
  const selectedField = this.selectedField_;
  if (!selectedField) {
    return -1;
  }
  for (let i = 0; i < this.inputList.length; i++) {
...
Blockly.ScratchBlocks.ProcedureUtils.finishAddingInput_ = function (
  ...
  let newIndex = this.inputList.length - 1;
  if (selectedIndex >= 0 && selectedIndex < newIndex) {
    const newInput = this.inputList.pop();
  ...
  const input = this.inputList[newIndex];
  if (input.type == Blockly.DUMMY_INPUT) {
...
Blockly.ScratchBlocks.ProcedureUtils.getUniqueArgumentName_ = function (
  ...
  for (let i = 0; i < this.inputList.length; i++) {
    const input = this.inputList[i];
    const target = input.connection && input.connection.targetBlock();
    const field = target && target.getField("TEXT");
    if (field) {
...
Blockly.ScratchBlocks.ProcedureUtils.argumentNameValidator_ = function (
  ...
  const argumentBlock = this.sourceBlock_;
  const declaration = argumentBlock && argumentBlock.parentBlock_;
...
Blockly.ScratchBlocks.ProcedureUtils.addLabelExternal = function () {
  const selectedIndex = this.getSelectedInputIndex_();
...
Blockly.ScratchBlocks.ProcedureUtils.removeFieldCallback = function (field) {
  ...
  const cannotRemove = (i) =>
    i == 0 && this.inputList[1].type == Blockly.NEXT_STATEMENT;
...
Blockly.ScratchBlocks.ProcedureUtils.updateArgumentReporterNames_ = function (
  ...
  let i, block, id, prevName, j, nameChange, k;
  const nameChanges = [];
  const argReporters = [];
  ...
  for (i = 0; (id = this.argumentIds_[i]); i++) {
    ...
    prevName = prevDisplayNames[prevIndex];
  ...
  for (j = 0; (nameChange = nameChanges[j]); j++) {
    for (k = 0; (block = nameChange.blocks[k]); k++) {
...
Blockly.Blocks["procedures_declaration"] = {
  ...
  getSelectedInputIndex_:
    Blockly.ScratchBlocks.ProcedureUtils.getSelectedInputIndex_,
  finishAddingInput_: Blockly.ScratchBlocks.ProcedureUtils.finishAddingInput_,
  getUniqueArgumentName_:
    Blockly.ScratchBlocks.ProcedureUtils.getUniqueArgumentName_,
  ...
  addStatementExternal:
    Blockly.ScratchBlocks.ProcedureUtils.addStatementExternal,
...
function argumentReporterMutationToDom() {
  if (!this.rendered || this.isShadow_)
    return document.createElement("mutation"); // Don't save the colour if we are a shadow.
