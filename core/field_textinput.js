...
Blockly.FieldTextInput.getTextMenuArrowAsset = function () {
  const textField = String(Blockly.Colours.textField || "")
    .trim()
    .toLowerCase();
  const textFieldText = String(Blockly.Colours.textFieldText || "")
    .trim()
    .toLowerCase();
  const defaultTheme = textField === "#ffffff" && textFieldText === "#575e75";
...
Blockly.FieldTextInput.prototype.init = function () {
  ...
  if (notInShadow) {
    const inProcedure = this.sourceBlock_.type === "procedures_declaration";
...
Blockly.FieldTextInput.prototype.render_ = function () {
  ...
  const lines = (this.text_ || "").split(/\r?\n/);
  const displayLines = [];
  let maxWidth = 0;
  for (let i = 0; i < lines.length; i++) {
    ...
    maxWidth = Math.max(
      maxWidth,
      Blockly.scratchBlocksUtils.measureText(
  ...
  let width = maxWidth + Blockly.BlockSvg.EDITABLE_FIELD_PADDING;
  ...
  this.textElement_.replaceChildren();
  const centerX = this.size_.width / 2;
  ...
  for (let j = 0; j < displayLines.length; j++) {
    const tspan = Blockly.utils.createSvgElement(
      "tspan",
      {
        x: centerX,
        dy:
...
Blockly.FieldTextInput.prototype.getMultilineHeight_ = function () {
  const lineCount = (this.text_ || "").split(/\r?\n/).length;
  return Math.max(
...
Blockly.FieldTextInput.prototype.showEditor_ = function (
  ...
  // Create the input.
  const htmlInput = this.multiline_
    ? goog.dom.createDom(
  ...
  if (opt_withArrow) {
    ...
    // Create the arrow.
    const dropDownArrow = goog.dom.createDom(
...
Blockly.FieldTextInput.prototype.onHtmlInputChange_ = function (e) {
  ...
  if (e.type === "keypress" && this.restrictor_) {
    ...
    if (isCharacter && !this.restrictor_.test(e.key)) {
...
Blockly.FieldTextInput.prototype.resizeEditor_ = function () {
  ...
  if (Blockly.BlockSvg.FIELD_TEXTINPUT_EXPAND_PAST_TRUNCATION) {
    ...
    const editorLines = Blockly.FieldTextInput.htmlInput_.value.split(/\r?\n/);
    let textWidth = 0;
  ...
  let editorHeight = Blockly.BlockSvg.FIELD_HEIGHT_MAX_EDIT;
  if (this.multiline_) {
    const editorLineCount =
      Blockly.FieldTextInput.htmlInput_.value.split(/\r?\n/).length;
