 ...
 * @constructor
 */
Blockly.FieldExtendable = function (
  elements,
  defaultInputs,
  minInputs,
  maxInputs,
  ...
  this.size_ = new goog.math.Size(
    ...
    Blockly.FieldExtendable.ARROW_WIDTH * 2
  );
...
Blockly.FieldExtendable.fromJson = function (options) {
  ...
  let defaultInputs = options.defaultInputs;
  if (defaultInputs === undefined) defaultInputs = 0;
  let minInputs = options.minInputs;
  if (minInputs === undefined) minInputs = 0;
  let maxInputs = options.maxInputs;
...
Blockly.FieldExtendable.prototype.getInsertIndex_ = function () {
  const inputList = this.sourceBlock_.inputList;
  const index = inputList.indexOf(this.sourceInput_);
...
Blockly.FieldExtendable.prototype.init = function () {
  ...
  this.arrowLeft = Blockly.utils.createSvgElement(
    "image",
    {
  ...
  this.arrowRight = Blockly.utils.createSvgElement(
    "image",
    {
      height: Blockly.FieldExtendable.ARROW_HEIGHT + "px",
      width: Blockly.FieldExtendable.ARROW_WIDTH + "px",
  ...
  this.arrowLeft.setAttributeNS(
    ...
    Blockly.mainWorkspace.options.pathToMedia +
      Blockly.FieldExtendable.ARROW_LEFT_PATH
  );
  this.arrowRight.setAttributeNS(
    "http://www.w3.org/1999/xlink",
    "xlink:href",
    Blockly.mainWorkspace.options.pathToMedia +
      Blockly.FieldExtendable.ARROW_RIGHT_PATH
  );
 ...
 */
Blockly.FieldExtendable.prototype.appendArgsList = function (
  elements,
  midfix,
  inputIndex,
  extendableIndex,
  ...
  const addedInputs = this.sourceBlock_.appendArgsList(elements, {
    position: inputIndex,
    returnInputs: true,
  ...
  for (let j = 0; j < addedInputs.length; j++) {
    const input = addedInputs[j];
 ...
 */
Blockly.FieldExtendable.prototype.setValue = function (
  newValue,
  opt_force,
  opt_noRender
) {
  let shouldPopulate, i, input, thisIndex;
  let newInputs = +newValue;
  if (!opt_force) {
  ...
  if (this.inputs !== newInputs) {
    shouldPopulate =
      this.sourceBlock_ &&
      (this.sourceBlock_.rendered || this.sourceBlock_.isInFlyout);
    ...
    if (this.sourceBlock_) {
      ...
      if (newInputs < this.inputs) {
        ...
        for (i = inputList.length - 1; i >= 0; i--) {
          ...
          if (
            input &&
            input.extendableName == this.name &&
            input.extendableIndex >= newInputs
          ) {
      ...
      thisIndex = this.getInsertIndex_();
      for (i = this.inputs; i < newInputs; i++) {
      ...
      if (newInputs === 0 && this.inputs !== 0) {
        ...
        // Or remove it
        for (i = this.sourceBlock_.inputList.length - 1; i >= 0; i--) {
          input = this.sourceBlock_.inputList[i];
          if (
            input &&
            input.extendableName == this.name &&
            input.extendableIndex == "COLLAPSER"
          ) {
...
Blockly.FieldExtendable.prototype.dispose = function () {
  ...
  if (this.sourceBlock_) {
    for (let i = this.sourceBlock_.inputList.length - 1; i >= 0; i--) {
