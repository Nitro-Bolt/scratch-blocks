 ...
 */
Blockly.FieldSlider.prototype.setConstraints = function (
  opt_min,
  opt_max,
  opt_precision
) {
  ...
  this.setRestrictor(
    this.getNumRestrictor(this.min_, this.max_, this.precision_)
  );
...
Blockly.FieldSlider.prototype.classValidator = function (value) {
  ...
  number =
    Math.round((number - this.min_) / this.precision_) * this.precision_ +
    this.min_;
  number = Math.min(this.max_, Math.max(this.min_, number));

  // Avoid floating-point noise such as 0.30000000000000004.
  const precisionText = String(this.precision_);
  const decimalPlaces =
    precisionText.indexOf(".") === -1
      ? 0
...
Blockly.FieldSlider.prototype.showEditor_ = function () {
  ...
  const parentBlock = this.sourceBlock_.parentBlock_ || this.sourceBlock_;
  slider.style.setProperty(
    "--blockly-slider-track-colour",
...
Blockly.FieldSlider.prototype.dropdownDispose_ = function () {
  for (let i = 0; i < this.sliderEventWrappers_.length; i++) {
