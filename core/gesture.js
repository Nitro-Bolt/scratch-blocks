...
Blockly.Gesture.prototype.doBlockClick_ = function () {
  ...
  if (this.mostRecentEvent_.button === 1 || this.mostRecentEvent_.shiftKey) {
    ...
    for (; block; block = block.getSurroundParent()) {
      ...
      const findProcCode = block.getProcCode();
      const topBlocks = this.startWorkspace_.getTopBlocks();
      for (let i = 0; i < topBlocks.length; i++) {
        const root = topBlocks[i];
...
Blockly.Gesture.prototype.isInputClick_ = function () {
  const inputClickable = this.startInput_
    ? this.startInput_.isClickable()
    : false;
...
Blockly.Gesture.prototype.duplicateOnDrag_ = function () {
  ...
  try {
    ...
    const xmlBlock = Blockly.Xml.blockToDom(this.targetBlock_, true);
