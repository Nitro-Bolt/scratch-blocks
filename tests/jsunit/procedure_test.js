...
function test_findCallers_noRecursion() {
  ...
  try {
    Blockly.Xml.domToWorkspace(Blockly.Xml.textToDom(xml), workspace);
    workspace.getBlockById("test_1").procCode_ = "test_procedure";
    const rootBlock = workspace.getBlockById("test_1");
    const callers = Blockly.Procedures.getCallers(
      "test_procedure",
      workspace,
      rootBlock,
...
function test_deleteProcedure_noCallers() {
  ...
  try {
    ...
    const rootBlock = workspace.getBlockById("test_1");
    assertTrue(
      Blockly.Procedures.deleteProcedureDefCallback("test_procedure", rootBlock)
    );
...
function test_deleteProcedure_nonRecursiveCaller() {
  ...
  try {
    Blockly.Xml.domToWorkspace(Blockly.Xml.textToDom(xml), workspace);
    workspace.getBlockById("test_1").procCode_ = "test_procedure";
    const rootBlock = workspace.getBlockById("test_2");
    assertFalse(
      Blockly.Procedures.deleteProcedureDefCallback("test_procedure", rootBlock)
    );
