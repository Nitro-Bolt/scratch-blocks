...
function verify_DB_(msg, expected, db) {
  let equal = expected.length == db.connections_.length;
  if (equal) {
...
function test_DB_addConnection() {
  const db = new Blockly.ConnectionDB();
  const o2 = {
    y_: 2,
    sourceBlock_: {},
...
function test_DB_getNeighbours() {
  ...
  let result = helper_getNeighbours(db, 0, 0, 4);
...
function test_DB_findPositionForConnection() {
  ...
  const conn = helper_createConnection(
    0,
    3,
...
function test_DB_findConnection() {
  ...
  let conn = helper_createConnection(
    3,
    3,
    Blockly.PREVIOUS_STATEMENT,
    null,
...
function test_SearchForClosest() {
  let db = new Blockly.ConnectionDB();
  const sharedWorkspace = { id: "Shared workspace" };
  ...
  const result = helper_searchDB(db, 4, 6, 3, sharedWorkspace);
...
function helper_searchDB(db, x, y, radius, shared_workspace) {
  const tempConn = helper_createConnection(
    x,
    y,
  ...
  const closest = db.searchForClosest(tempConn, radius, { x: 0, y: 0 });
...
function helper_createConnection(
  ...
  const workspace = opt_shared_workspace ? opt_shared_workspace : {};
  if (opt_rendered) {
