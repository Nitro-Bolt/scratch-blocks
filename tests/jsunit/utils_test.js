...
function test_genUid() {
  ...
  for (let i = 0; i < 1000; i++) {
    const uuid = Blockly.utils.genUid();
...
function test_removeClass() {
  ...
  assertEquals('Removing "two"', "one three", p.className);
  Blockly.utils.removeClass(p, "four");
  assertEquals('Removing "four"', "one three", p.className);
...
function test_tokenizeInterpolation() {
  let tokens = Blockly.utils.tokenizeInterpolation("");
