...
function test_block_base_constructor() {
  ...
  try {
    const block = createSimpleTestBlock(workspace);

    // Here's the event we care about.
    const event = new Blockly.Events.BlockBase(block);
...
function test_var_base_constructor() {
  ...
  setUpMockMethod(mockControl_, Blockly.utils, "genUid", null, "1");
  try {
    const variable = workspace.createVariable("name1", "type1", "id1");

    const event = new Blockly.Events.VarBase(variable);
...
function checkCreateEventValues(event, block, ids, type) {
  const expected_xml = Blockly.Xml.domToText(Blockly.Xml.blockToDom(block));
  const result_xml = Blockly.Xml.domToText(event.xml);
...
function checkDeleteEventValues(event, block, ids, type) {
  const expected_xml = Blockly.Xml.domToText(Blockly.Xml.blockToDom(block));
  const result_xml = Blockly.Xml.domToText(event.oldXml);
  assertEquals(expected_xml, result_xml);
  isEqualArrays(ids, event.ids);
  assertEquals(type, event.type);
}
...
function createSimpleTestBlock(workspace) {
  ...
  const block = new Blockly.Block(workspace, "simple_test_block");
...
function test_create_constructor() {
  ...
  try {
    ...
    const event = new Blockly.Events.Create(block);
...
function test_delete_constructor() {
  ...
  try {
    const block = createSimpleTestBlock(workspace);
    const event = new Blockly.Events.Delete(block);
...
function test_change_constructor() {
  ...
  try {
    Blockly.Events.disable();
    const block = new Blockly.Block(workspace, "field_variable_test_block");
    Blockly.Events.enable();
...
function test_move_constructorCoordinate() {
  ...
  try {
    const block1 = createSimpleTestBlock(workspace);
    const coordinate = new goog.math.Coordinate(3, 4);
    block1.xy_ = coordinate;

    const event = new Blockly.Events.Move(block1);
...
function test_move_constructoroldParentId() {
  ...
  try {
    const block1 = createSimpleTestBlock(workspace);
    const block2 = createSimpleTestBlock(workspace);
...
function test_blockMove_constructorCoordinate() {
  ...
  try {
    ...
    const event = new Blockly.Events.BlockMove(block1);
    // Need to check for individual equality of the coordinate values since
    // the move event creates a new goog.math.Coordinate object
    assertEquals(event.oldCoordinate.x, coordinate.x);
    assertEquals(event.oldCoordinate.y, coordinate.y);
...
function test_uiEvent_constructor_block() {
  ...
  try {
    const block1 = createSimpleTestBlock(workspace);
    Blockly.Events.setGroup("testGroup");
    const event = new Blockly.Events.Ui(block1, "foo", "bar", "baz");
    checkExactEventValues(event, {
      blockId: "1",
      workspaceId: workspace.id,
      type: "ui",
...
function test_varCreate_constructor() {
  ...
  try {
    const variable = workspace.createVariable("name1", "type1", "id1");
    const event = new Blockly.Events.VarCreate(variable);
    checkExactEventValues(event, {
      varName: "name1",
      varType: "type1",
      type: "var_create",
...
function test_varCreate_toJson() {
  ...
  try {
    ...
    const localVariable = workspace.createVariable(
      "name2",
      "type2",
      "id2",
      true
    );
    const event2 = new Blockly.Events.VarCreate(localVariable);
    const json2 = event2.toJson();
    const expectedJson2 = {
      type: "var_create",
      varId: "id2",
      varType: "type2",
    ...
    const cloudVariable = workspace.createVariable(
      "name3",
      "type3",
      "id3",
      false,
    ...
    const event3 = new Blockly.Events.VarCreate(cloudVariable);
    const json3 = event3.toJson();
    const expectedJson3 = {
      type: "var_create",
      varId: "id3",
      varType: "type3",
...
function test_varCreate_fromJson() {
  ...
  try {
    const variable = workspace.createVariable("name1", "type1", "id1");
    const event = new Blockly.Events.VarCreate(variable);
    const event2 = new Blockly.Events.VarCreate(null);
    const json = event.toJson();
...
function test_varCreate_runForward() {
  ...
  const event = Blockly.Events.fromJson(json, workspace);
...
function test_varDelete_constructor() {
  eventTest_setUp();
  const variable = workspace.createVariable("name1", "type1", "id1");
  const event = new Blockly.Events.VarDelete(variable);
  checkExactEventValues(event, {
    varName: "name1",
    varType: "type1",
    varId: "id1",
...
function test_varDelete_toJson() {
  ...
  const localVariable = workspace.createVariable("name2", "type2", "id2", true);
  const event2 = new Blockly.Events.VarDelete(localVariable);
  const json2 = event2.toJson();
  const expectedJson2 = {
    type: "var_delete",
    varId: "id2",
  ...
  const event3 = new Blockly.Events.VarDelete(cloudVariable);
  const json3 = event3.toJson();
  const expectedJson3 = {
    type: "var_delete",
    varId: "id3",
...
function test_varDelete_fromJson() {
  ...
  const variable = workspace.createVariable("name1", "type1", "id1");
  const event = new Blockly.Events.VarDelete(variable);
  const event2 = new Blockly.Events.VarDelete(null);
  const json = event.toJson();
  event2.fromJson(json);
...
function test_varRename_constructor() {
  eventTest_setUp();
  const variable = workspace.createVariable("name1", "type1", "id1");
  const event = new Blockly.Events.VarRename(variable, "name2");
  checkExactEventValues(event, {
    varId: "id1",
    oldName: "name1",
    newName: "name2",
...
function test_varRename_fromJson() {
  eventTest_setUp();
  const variable = workspace.createVariable("name1", "type1", "id1");
  const event = new Blockly.Events.VarRename(variable, "");
  const event2 = new Blockly.Events.VarRename(null);
  const json = event.toJson();
  event2.fromJson(json);
...
function test_events_filter() {
  eventTest_setUpWithMockBlocks();
  try {
    const block1 = workspace.newBlock("field_variable_test_block", "1");
    const events = [
...
function test_events_filterForward() {
  ...
  try {
    const block1 = workspace.newBlock("field_variable_test_block", "1");
    const events = [new Blockly.Events.BlockCreate(block1)];
...
function test_events_filterDifferentBlocks() {
  eventTest_setUpWithMockBlocks();
  const block1 = workspace.newBlock("field_variable_test_block", "1");
  const block2 = workspace.newBlock("field_variable_test_block", "2");
  const events = [
    new Blockly.Events.BlockCreate(block1),
    new Blockly.Events.BlockMove(block1),
...
function test_events_mergeUi() {
  ...
  const block1 = workspace.newBlock("field_variable_test_block", "1");
  const block2 = workspace.newBlock("field_variable_test_block", "2");
  const block3 = workspace.newBlock("field_variable_test_block", "3");
  const events = [
...
function test_events_stackclick() {
  eventTest_setUpWithMockBlocks();
  const block = workspace.newBlock("field_variable_test_block", "1");
  const events = [
...
function test_events_newblock_newvar() {
  ...
  try {
    const block = workspace.newBlock("field_variable_test_block");

    const firedEvents = workspace.undoStack_;
    // Expect two events: varCreate and block create.
    assertEquals(2, firedEvents.length);

    const event0 = firedEvents[0];
    const event1 = firedEvents[1];
